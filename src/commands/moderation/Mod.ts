...
export default class Mod extends BaseCommand {
    constructor() {
        super({
            category: "moderation",
            description: "Add a role/user to the ignored list for moderation actions",
            name: "mod",
    ...
    public async run(client: BaseClient, message: Message, args: string[]) {
        ...
        if (!guild) guild = new Guild({ gId: message.guild.id });
        ...
        switch (add_remove) {
            case "add":
                ...
                guild.modRoles_Users.push(member_role.id);
            ...
            case "remove":
                ...
                guild.modRoles_Users.splice(guild.modRoles_Users.indexOf(member_role.id), 1);
